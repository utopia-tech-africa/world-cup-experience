...
export function ContactToggle() {
  ...
  useEffect(() => {
    ...
    const hasBeenShown = sessionStorage.getItem("contact-auto-popup-shown");
  ...
  useEffect(() => {
    ...
    const handleAutoTrigger = () => {
      setIsModalOpen(true);
      setHasTriggered(true);
      sessionStorage.setItem("contact-auto-popup-shown", "true");
    };

    // Scroll detection for middle of site and packages section
    ...
    const handleScroll = () => {
      ...
      // Trigger when user has scrolled through 40% of the page (roughly "middle" or approaching middle)
      const triggerPoint = (fullHeight - viewportHeight) * 0.45;
...
const ContactWidget = ({ onOpenModal }: { onOpenModal: () => void }) => {
  return (
    <div className="w-full max-w-[320px] md:max-w-[380px] bg-black/40 backdrop-blur-xl border-6 border-white/30 rounded-lg p-4 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        <button
          ...
          className="group flex items-center justify-between gap-1 p-3 rounded-lg bg-black/50 border border-white/10 transition-all hover:bg-white hover:border-white cursor-pointer"
        >
          <ContactItemContent
            icon={MessageSquare}
            label="MESSAGE US"
...
const ContactItem = ({
  ...
  >
    <ContactItemContent icon={Icon} label={label} value={value} />
  </a>
);
